...
class TensorAttention(Layer):
    ...
    def call(self, X, mask=None):
        ...
        if self.context == 'word':
            ...
            def step(X, states):
                new_state = activations.tanh(tf.tensordot(X,self.encoder_weight, axes=[[2],[0]]) + tf.tensordot(states[0], self.recurrent_weight, axes=[[2],[0]]))
                return new_state, [new_state]

            # Make all-zero initial state.
            # Directly obtaining the first input dimension is not allowed, so this is the work-around.
            ...
            raw_att_scores = tf.tensordot(K.permute_dimensions(all_rnn_out, (0, 2, 1, 3)), self.att_scorer, axes=[[3],[0]])
        
        elif self.context == 'bidirectional_clause':
            def step_forward(X, states):
                new_state = activations.tanh(tf.tensordot(X,self.encoder_weight_forward, axes=[[2],[0]]) + tf.tensordot(states[0], self.recurrent_weight_forward, axes=[[2],[0]]))
                return new_state, [new_state]
            def step_backward(X, states):
                new_state = activations.tanh(tf.tensordot(X,self.encoder_weight_backward, axes=[[2],[0]]) + tf.tensordot(states[0],self.recurrent_weight_backward, axes=[[2],[0]]))
                return new_state, [new_state]
            # Make all-zero initial state. 
            ...
            def step(inputs, states):
                ...
                i = activations.hard_sigmoid(x_i + tf.tensordot(h_tm1, self.recurrent_kernel_i,axes=[[2],[0]]))
                f = activations.hard_sigmoid(x_f + tf.tensordot(h_tm1, self.recurrent_kernel_f,axes=[[2],[0]]))
                c = f * c_tm1 + i * activations.tanh(x_c + tf.tensordot(h_tm1, self.recurrent_kernel_c,axes=[[2],[0]]))
                o = activations.hard_sigmoid(x_o + tf.tensordot(h_tm1, self.recurrent_kernel_o,axes=[[2],[0]]))
            ...
            def step_forward(inputs, states):
                ...
                i = activations.hard_sigmoid(x_i + tf.tensordot(h_tm1, self.recurrent_kernel_i_forward,axes=[[2],[0]]))
                f = activations.hard_sigmoid(x_f + tf.tensordot(h_tm1, self.recurrent_kernel_f_forward,axes=[[2],[0]]))
                c = f * c_tm1 + i * activations.tanh(x_c + tf.tensordot(h_tm1, self.recurrent_kernel_c_forward,axes=[[2],[0]]))
                o = activations.hard_sigmoid(x_o + tf.tensordot(h_tm1, self.recurrent_kernel_o_forward,axes=[[2],[0]]))
                h = o * activations.tanh(c)
            ...
            def step_backward(inputs, states):
                ...
                i = activations.hard_sigmoid(x_i + tf.tensordot(h_tm1, self.recurrent_kernel_i_backward,axes=[[2],[0]]))
                f = activations.hard_sigmoid(x_f + tf.tensordot(h_tm1, self.recurrent_kernel_f_backward,axes=[[2],[0]]))
                c = f * c_tm1 + i * activations.tanh(x_c + tf.tensordot(h_tm1, self.recurrent_kernel_c_backward,axes=[[2],[0]]))
                o = activations.hard_sigmoid(x_o + tf.tensordot(h_tm1, self.recurrent_kernel_o_backward,axes=[[2],[0]]))
                h = o * activations.tanh(c)
        ...
        if self.return_attention:
            if tf.__version__.startswith('2'):
                result_shape, att_shape = self.compute_output_shape(result.shape)
