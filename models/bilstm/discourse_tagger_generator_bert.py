...
class PassageTagger(object):
    ...
    def fit_model(self, train_generator, validation_generator, reg=0):
        ...
        if use_attention:
            ...
            x, raw_attention = TensorAttention(att_input_shape, context=att_context, hard_k=hard_k,
                                               proj_dim=att_proj_dim, rec_hid_dim=rec_hid_dim, return_attention=True)(x)
        ...
        def step_decay(current_epoch):
            ...
            drop = 0.5
            epochs_drop = epoch / 2
            lrate = initial_lrate * np.power(drop, np.floor((1 + current_epoch) / epochs_drop))
            return lrate
...
if __name__ == "__main__":
    ...
    if params["train"]:
        ...
        if params["repfile"]:
            ...
            _, validation_generator = nnt.make_data(
